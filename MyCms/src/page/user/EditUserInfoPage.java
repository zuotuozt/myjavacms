...
public class EditUserInfoPage extends TransactionPage {
	...
	public String print(HttpServletRequest req, HttpServletResponse resp)
		...
		if(transactionConn(args)){//修改用户信息
		}else{
			PubFun.ajaxPrintStr("修改用户信息失败。", resp);
			return null;
		}
	...
	protected void handleTransaction(Connection conn, Object[] args)
		...
		UserInfoTable.updateUserInfo(conn, userInfo);

		String cols = (String) args[1];;
		if(!cols.equals("")){			
			UserRoleTable.delUserRole(conn, userInfo.getId());
			UserRoleTable.batchInsert(conn, cols.split(","), userInfo.getId());
		}		
	}
